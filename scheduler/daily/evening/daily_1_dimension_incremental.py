...
from utils.rls_utils import update_rls_master
...
async def process_dimension_table(
    ...
    try:
        ...
        table_name = table_mapping.get(base_table_name, base_table_name.lower())
        ...
        if not success:
            logger.error(
                f"{RED}❌ ETL failed: {etl_result.get('error', 'Unknown error')}{RESET}"
            )
            return {
        ...
        # Special handling for DimHierarchy - update RLS Master
        if table_name == "dim_hierarchy":
            logger.info("� Updating RLS Master...")
            try:
                rls_count = update_rls_master(logger)
                logger.info(f"{GREEN}✓ RLS Master updated with {rls_count:,} records{RESET}")
            except Exception as e:
                logger.error(f"{RED}⚠️  RLS Master update failed: {e}{RESET}")
        
        logger.info(
            f"{GREEN}✅ Successfully loaded {rows_loaded:,} rows in {elapsed:.2f}s{RESET}"
        )
