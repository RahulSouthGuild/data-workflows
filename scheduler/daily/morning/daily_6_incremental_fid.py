...
from utils.pipeline_config import Config, DAILY_FID_INCREMENTAL_SERVICE_NAME, LOG_SEPARATOR
...
async def create_connection_pool():
    """StarRocks connections handled per-request via get_starrocks_connection()"""
    return None  # Not needed for sync MySQL
...
def get_table_count(table_name="fact_invoice_details"):
...
async def delete_chunk(records, record_bar, task_logger, attempt=1):
    ...
    try:
        ...
        for r in records:
            composite_id = (
                (str(r["posting_date"]) if r.get("posting_date") is not None else "NULL")
                + "_"
                ...
                + "_"
                + (str(r["mis_type"]) if r.get("mis_type") is not None else "NULL")
                + "_"
                + (str(r["item_number"]) if r.get("item_number") is not None else "NULL")
            )
        ...
        try:
            with conn.cursor() as cursor:
                ...
                for batch_start in range(0, len(unique_composite_ids), BATCH_SIZE):
                    ...
                    # Incremental ingestion: delete ANY record with matching fid_pd_cc_in_mt_in
                    placeholders = ", ".join(["%s"] * len(batch_ids))
                    query = f"""
                        DELETE FROM fact_invoice_details
                        WHERE fid_pd_cc_in_mt_in IN ({placeholders})
                    """
...
async def bulk_delete(df, task_logger):
    ...
    if total_records > 0:
        ...
        task_logger.debug(
            f"   Sample: posting_date={sample_record.get('posting_date')}, "
    ...
    async with sem:
        try:
            ...
            success, result = stream_load_csv(
                "fact_invoice_details", csv_path, columns=db_column_order
            )
...
async def insert_data(df, task_logger):
    ...
    # Note: fid_pd_cc_in_mt_in is already generated during transform_dataframe()
    ...
    try:
        conn = get_starrocks_connection()
        with conn.cursor() as cursor:
            cursor.execute("DESC fact_invoice_details")
...
async def process_file(filename, task_logger, main_logger=None):
    """Process FID file with lock management"""
    try:
        start_time = time.time()
        ...
        try:
            final_table_count = get_table_count()
...
async def transform_dataframe(df, schema_info, stem, task_logger):
    ...
    try:
        ...
        # Extract table name from file stem (FactInvoiceDetails_... -> fact_invoice_details)
...
@with_status_tracking(DAILY_FID_INCREMENTAL_SERVICE_NAME)
async def run_incremental_fid(main_logger=None):
    task_logger = get_pipeline_logger(DAILY_FID_INCREMENTAL_SERVICE_NAME)
    total_start_time = time.time()
    try:
        task_logger.info(LOG_SEPARATOR)
        task_logger.info("🚀 Starting FID incremental job")
        task_logger.info(LOG_SEPARATOR)
        ...
        fid_files = os.listdir(PROJECT_ROOT / "data" / "data_incremental" / "cleaned_parquets")
        if fid_files:
            ...
            task_logger.info(f"🔄 Found {len(fid_files)} files to process")
...
if __name__ == "__main__":
    ...
    task_logger.info("🚀 Starting FID incremental job...")
    task_logger.info(LOG_SEPARATOR)
    asyncio.run(run_incremental_fid())
