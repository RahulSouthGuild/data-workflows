#!/usr/bin/env python3
"""
Pidilite - Daily Dimension Incremental Load Job

Loads dimension tables for Pidilite tenant from Azure Blob Storage to StarRocks.
This is a tenant-specific job that uses Pidilite's configuration.
"""
...
async def main():
    ...
    # Set up logging
    logger = get_pipeline_logger(f"pidilite_dimension_incremental_{int(time.time())}")
    ...
    try:
        ...
        async with blob_service_client:
            ...
            logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
            logger.info(f"{CYAN}PIDILITE DIMENSION LOAD SUMMARY{RESET}")
            logger.info(f"{CYAN}{'=' * 80}{RESET}")
