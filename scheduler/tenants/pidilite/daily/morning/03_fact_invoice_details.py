#!/usr/bin/env python3
"""
Pidilite - Daily Fact Invoice Details Incremental Load

Loads fact_invoice_details table for Pidilite tenant from Azure to StarRocks.
"""
...
async def main():
    ...
    # Set up logging
    logger = get_pipeline_logger(f"pidilite_fid_incremental_{int(time.time())}")

    logger.info("=" * 80)
    logger.info(f"{CYAN}PIDILITE - DAILY FACT INVOICE DETAILS INCREMENTAL{RESET}")
    logger.info("=" * 80)
    ...
    try:
        ...
        async with blob_service_client:
            ...
            logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
            logger.info(f"{CYAN}PIDILITE FACT INVOICE DETAILS LOAD SUMMARY{RESET}")
            logger.info(f"{CYAN}{'=' * 80}{RESET}")
