#!/usr/bin/env python3
"""
Uthra Global - Daily Dimension Incremental Load Job

Loads dimension tables for Uthra Global tenant from Azure Blob Storage to StarRocks.
This is a tenant-specific job that uses Uthra Global's configuration.
"""
...
async def main():
    ...
    # Set up logging
    logger = get_pipeline_logger(f"uthra_global_dimension_incremental_{int(time.time())}")

    logger.info("=" * 80)
    logger.info(f"{CYAN}PIDILITE - DAILY DIMENSION INCREMENTAL LOAD{RESET}")
    logger.info("=" * 80)
    ...
    try:
        ...
        async with blob_service_client:
            ...
            for table_name, azure_folder in DIMENSION_TABLES:
                logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
                logger.info(f"{CYAN}Processing: {table_name}{RESET}")
            ...
            logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
            logger.info(f"{CYAN}UTHRA GLOBAL DIMENSION LOAD SUMMARY{RESET}")
            logger.info(f"{CYAN}{'=' * 80}{RESET}")
        ...
        logger.error(f"{RED}❌ Fatal error in dimension load job: {str(e)}{RESET}")
        logger.exception(e)
        sys.exit(1)
