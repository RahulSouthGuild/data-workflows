#!/usr/bin/env python3
"""
Uthra Global - Daily Fact Invoice Secondary Incremental Load

Loads fact_invoice_secondary table for Uthra Global tenant from Azure to StarRocks.
"""
...
async def main():
    ...
    # Set up logging
    logger = get_pipeline_logger(f"uthra_global_fis_incremental_{int(time.time())}")

    logger.info("=" * 80)
    logger.info(f"{CYAN}UTHRA GLOBAL - DAILY FACT INVOICE SECONDARY INCREMENTAL{RESET}")
    logger.info("=" * 80)
    ...
    try:
        ...
        async with blob_service_client:
            ...
            logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
            logger.info(f"{CYAN}UTHRA GLOBAL FACT INVOICE SECONDARY LOAD SUMMARY{RESET}")
            logger.info(f"{CYAN}{'=' * 80}{RESET}")
