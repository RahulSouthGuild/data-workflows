#!/usr/bin/env python3
"""
Uthra Global - Daily Fact Invoice Details Incremental Load

Loads fact_invoice_details table for Uthra Global tenant from Azure to StarRocks.
"""
...
# Azure folders for fact_invoice_details with folder mapping
# Format: (azure_folder_name,)
FACT_FOLDERS = [
    "FactInvoiceDetails",
    "FactInvoiceDetails_107_112",
]
...
async def main():
    ...
    # Set up logging
    logger = get_pipeline_logger(f"uthra_global_fid_incremental_{int(time.time())}")

    logger.info("=" * 80)
    logger.info(f"{CYAN}UTHRA GLOBAL - DAILY FACT INVOICE DETAILS INCREMENTAL{RESET}")
    logger.info("=" * 80)
    ...
    try:
        ...
        async with blob_service_client:
            ...
            for azure_folder in FACT_FOLDERS:
                logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
                logger.info(f"{CYAN}Processing: fact_invoice_details from {azure_folder}{RESET}")
            ...
            logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
            logger.info(f"{CYAN}UTHRA GLOBAL FACT INVOICE DETAILS LOAD SUMMARY{RESET}")
            logger.info(f"{CYAN}{'=' * 80}{RESET}")
        ...
        logger.error(f"{RED}❌ Fatal error in FID load job: {str(e)}{RESET}")
        logger.exception(e)
        sys.exit(1)
