#!/usr/bin/env python3
"""
Uthra Global - Evening Dimension Tables Refresh

Refreshes dimension tables in the evening for Uthra Global tenant.
This ensures dimension data is up-to-date before overnight processing.
"""
...
# Dimension tables to refresh with Azure folder mapping
...
async def process_dimension_table(
    ...
    Returns:
        dict: Processing result with success/failure status
    """
    start_time = time.time()

    try:
        result = await orchestrator.process_parquet(
        ...
        logger.error(f"{RED}❌ Failed to process {parquet_path.name}: {str(e)}{RESET}")
        return {
            "success": False,
...
async def main():
    ...
    # Set up logging
    logger = get_pipeline_logger(f"uthra_global_dimensions_evening_{int(time.time())}")

    logger.info("=" * 80)
    logger.info(f"{CYAN}PIDILITE - EVENING DIMENSION REFRESH{RESET}")
    logger.info("=" * 80)
    ...
    try:
        ...
        async with blob_service_client:
            ...
            for table_name, azure_folder in DIMENSION_TABLES:
                logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
                logger.info(f"{CYAN}Refreshing: {table_name}{RESET}")
                logger.info(f"{CYAN}{'=' * 80}{RESET}")

                # Determine Azure folder path for this table
                ...
                for parquet_path in blob_result["successful"]:
                    try:
                        success, result = orchestrator.orchestrate(
                            ...
                            truncate=True  # Evening refresh, truncate and reload
                        )
                        if success:
            ...
            logger.info(f"\n{CYAN}{'=' * 80}{RESET}")
            logger.info(f"{CYAN}UTHRA GLOBAL EVENING DIMENSION REFRESH SUMMARY{RESET}")
            logger.info(f"{CYAN}{'=' * 80}{RESET}")
        ...
        logger.error(f"{RED}❌ Fatal error in evening dimension refresh: {str(e)}{RESET}")
        logger.exception(e)
        sys.exit(1)
